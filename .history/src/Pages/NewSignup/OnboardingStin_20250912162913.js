...
function OnboardingStin() {
  ...
  const location = useLocation();
    const sTins = location.state?.sTins || [];
...
const [activeIndex, setActiveIndex] = useState(0);
      ...
      <div className={classes.lftcontainer}>
        ...
        <div className={classes.rgtcontainer}>
          <div className={classes.maintext}>
            <h1> S-TIN Verification </h1>
            <h6>{userType === 'individual' ? 'To continue, please select your personal S-TIN (State Tax Identification Number) from the dropdown below or generate if you dont have one.' :   'To continue, please select your Company S-TIN (State Tax Identification Number) from the dropdown below or generate if you dont have one.'}</h6>
  ...
  <div className={`mb-3 ${classes.stintext}`} style={{ marginTop: 10 }}>
    ...
    {sTins.length > 0 ? (
      // ✅ Case 1: S-TINs exist → show select dropdown
      <Form.Select
        disabled={fetchData === true}
        id="sTin"
        style={{ padding: "10px", height: "46px", width: "100%" }}
      ...
      <input
        ...
        readOnly
        disabled
        placeholder="No S-TIN available, please generate one"
        style={{ padding: "10px", height: "46px", width: "100%" }}
        className="form-control"
      />
    )}
  ...
  {/* Conditional “don’t have S-TIN” link */}
  {tinLoading ? (
    <p
      style={{
        textAlign: "left",
    ...
    >
      {sTins.length > 0 ? null : (
        <>
          ...
          >
            Click here to generate an S-TIN
          </span>
        </>
      )}
    </p>
